...
Future<List<HistoryPoint>> loadHistoryPoints() async {
  ...
  await rootBundle.loadString(DataAssets.jsonHistoryPoint);
...
class _HistoryPointScreenState extends State<HistoryPointScreen> {
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  SliverPersistentHeader(
                      pinned: true, delegate: _HeaderDelegate()),
  ...
  Widget _buildBarcodeWidget() {
    final scannedBarcode = "1234567890";
    final isNumeric = RegExp(r'^\d+$').hasMatch(scannedBarcode);

    return barcode_widget.BarcodeWidget(
      barcode:
      isNumeric ? barcode_widget.Barcode.code128() : barcode_widget.Barcode.qrCode(),
      data: scannedBarcode,
      width: 200,
      height: 80,
      color: Colors.black,
